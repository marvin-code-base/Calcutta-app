...
export default async function handler(req, res) {
  ...
  try {
    ...
    // NOTE: site.api.espn.com/apis/site/v2/.../standings is a known dead end —
    // it returns a stub with no real entries for several sports including
    // NFL. site.web.api.espn.com/apis/v2/... is the endpoint that actually
    // carries data, and it needs an explicit season year.
    const espnRes = await fetch(
      `https://site.web.api.espn.com/apis/v2/sports/football/nfl/standings?season=${league.season_year}&level=1`
    );
    if (!espnRes.ok) {
    ...
    // Walk the standings structure to pull { abbreviation, wins, ties } per
    // team. level=1 should return a flat list at the top, but fall back to
    // a grouped/children shape defensively in case ESPN's response varies.
    const records = [];
    const flatEntries = espnData.standings?.entries || [];
    const groupedEntries = (espnData.children || []).flatMap(
      (g) => g.standings?.entries || []
    );
    const entries = flatEntries.length > 0 ? flatEntries : groupedEntries;
    ...
    if (records.length === 0) {
      throw new Error(
        "ESPN response didn't contain any standings entries — the season may not have started, or ESPN's response shape changed."
      );
    }
